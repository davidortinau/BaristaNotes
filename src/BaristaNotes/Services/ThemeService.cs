...
#endif
{
...
#if DEBUG
        // Register with the HotReload registry so OnHotReload() fires when
        // ThemeService's type is updated by .NET Hot Reload. Generated by
...
#if DEBUG
    /// <summary>
    /// Fired by Microsoft.Maui.Labs.HotReload's MetadataUpdateHandler when this
    /// instance's type (ThemeService) is updated by .NET Hot Reload. The
    /// <paramref name="updatedTypes"/> array contains all types the runtime
    /// reported as changed in this update — useful for discriminating.
    /// </summary>
    public void OnHotReload(Type[]? updatedTypes)
    {
        ...
        _logger.LogInformation("🔥 ThemeService.OnHotReload fired. Updated types: {Types}", names);
    }
#endif
}
